...
moyenne_modeles_stepAIC <- function(
  ...
  for (b in seq_len(B)) {
    ...
    modselect <- MASS::stepAIC(
      ...
      object = lm(
        ...
        data = matmod[sample.int(n = N, size = N, replace = TRUE), ]
      ),
      ...
      keep = function(mod, AIC, ...) {
        ...
        list(IC = AIC(mod, k = ifelse(aic, 2, log(N))), coef = coef(mod))
      },
      k = ks
    )
    ...
    colind <- match(names(coefsv), names(params))
  ...
  names(nselect) <- noms
  names(params) <- noms
  return(list(coefs = params / B, nselect = nselect[-1] / B))
}
...
eval_EQM_regsubsets <-
  function(
    ...
    if (select_ic) {
      ...
      select_id <- c(which.min(aic), which.min(bic))[c(
        "AIC" %in% select,
        "BIC" %in% select
      )]
      select <- sort(select)
    ...
    stopifnot(
      all(colnames(model_matrix) == var_names),
    ...
    for (i in seq_along(select_id)) {
      ...
      new_formula <- paste0("reponse~", paste(var_names[vlist], collapse = "+"))
      linmod <- lm(new_formula, data = model_matrix)
      pred <- predict(linmod, newdata = valid_matrix)
...
predict.regsubsets <- function(object, newdata, id, ...) {
  id <- as.integer(id)
  stopifnot(
    isTRUE(inherits(object, "regsubsets")),
...
AIC.summary.regsubsets <- function(object, ..., k = 2) {
  ...
  ll <- object$obj
  ll$nn *
    (log(object$rss) - log(ll$nullrss)) +
    k * ll$first:min(ll$last, ll$nvmax)
}
